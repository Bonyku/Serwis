...
export class CartService {
  private items$: ReplaySubject<ProductInCart[]> = new ReplaySubject<
    ProductInCart[]
  >(1);
  ...
  public count(): Observable<number> {
    return this.items$.pipe(
      map((i: ProductInCart[]) =>
        i.reduce((acc: number, curr: ProductInCart) => acc + curr.quantity, 0)
      )
    );
  }
  ...
  public add(product: Product): void {
    const prod: ProductInCart | undefined = this._items.find(
